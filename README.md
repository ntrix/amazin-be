...
| `REDIS_URL` | Login-lockout counter/lock, shared across every BE instance | Local dev: `docker compose up` already starts one at `redis://localhost:6379` (see below). Production: any managed Redis (e.g. AWS ElastiCache). Unset or unreachable → fails open, lockout is simply skipped, login itself still works |
